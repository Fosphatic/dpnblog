...
class CommentApiController
{
    ...
    public function indexAction($filter = [], $post = 0, $page = 0, $limit = 0)
    {
        ...
        } elseif (!$this->user->hasAccess('dpnblog: manage comments')) {
            App::abort(403, __('Insufficient user rights.'));
        }

        if (!$this->user->hasAccess('dpnblog: manage comments')) {
        ...
        if (preg_match('/^(created)\s(asc|desc)$/i', $order, $match)) {
            ...
            $order = [1 => 'created', 2 => App::module('dpnblog')->config('comments.order')];
        }
        ...
        foreach ($comments as $i => $comment) {
            ...
            if ($post && (!$p || !$p->hasAccess($this->user) || !($p->isPublished() || $this->user->hasAccess('dpnblog: manage comments')))) {
    ...
    public function saveAction($data, $id = 0)
    {
        if (!$id) {
            ...
            if (!$this->user->hasAccess('dpnblog: post comments')) {
                App::abort(403, __('Insufficient User Rights.'));
            }
        ...
        if (!$this->user->hasAccess('dpnblog: skip comment min idle')
        ...
        if (!@$data['post_id'] || !$post = Post::where(['id' => $data['post_id']])->first() or !($this->user->hasAccess('dpnblog: manage comments') || $post->isCommentable() && $post->isPublished())) {
            App::abort(404, __('Post not found.'));
        }

        $approved_once = (boolean) Comment::where(['user_id' => $this->user->id, 'status' => Comment::STATUS_APPROVED])->first();
        $comment->status = $this->user->hasAccess('dpnblog: skip comment approval') ? Comment::STATUS_APPROVED : $this->user->hasAccess('dpnblog: comment approval required once') && $approved_once ? Comment::STATUS_APPROVED : Comment::STATUS_PENDING;
